<template>
  <template v-if="!menu.hideMenu">
    <el-sub-menu v-if="showMenuType === 2" :index="pathResolve">
      <template #title>
        <component v-if="menu.meta.icon" :is="useRenderIcon(menu.meta.icon, 'svg')" />
    ...
    <app-link v-else-if="showMenuType === 1" :to="pathResolve">
      <el-menu-item :index="pathResolve" v-if="!menu.children[0].children || menu.children[0].children.length === 0">
        <component v-if="menu.children[0].meta.icon || menu.meta.icon" :is="useRenderIcon(menu.children[0].meta.icon || menu.meta.icon, 'svg')" />
